'''
PennyLane version of the code that implements the quantum circuit in: 
...
def circuit():
    ...
    for gridRow in range(N):
        ...
        for gridColumn in range(0,N-1):    
            ...
            if gridColumn%2==0:
                qml.CZ([gridColumn+gridRow*N,gridColumn+gridRow*N+1])
    ...
    for gridColumn in range(N):
        for gridRow in range(0,N-1):
            if gridRow%2==0:
                qml.CZ([gridColumn+gridRow*N,gridColumn+(gridRow+1)*N])
...
#execute the circuit & store its output
z = circuit()
