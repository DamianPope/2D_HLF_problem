'''
TKET version of the code that implements the quantum circuit in: 

    Quantum Advantage with Shallow Circuits 
    Sergey Bravyi, David Go'sset, Robert Koenig. Science 362 (6412) pp. 308-311, 2018
...
c = Circuit(n, n) # define a circuit with n qubits and n bits
...
for gridRow in range(N):
    ...
    for gridColumn in range(0,N-1):    
        ...
        if gridColumn%2==0:
            c.CZ(gridColumn+gridRow*N,gridColumn+gridRow*N+1)
...
for gridColumn in range(N):
    for gridRow in range(0,N-1):
        if gridRow%2==0:
            c.CZ(gridColumn+gridRow*N,gridColumn+(gridRow+1)*N)
...
#Create backend & execute the circuit on it
backend = AerBackend()

compiled_circ = backend.get_compiled_circuit(c)

#Note that handle is a label that identifies the output of the completed job
handle = backend.process_circuit(compiled_circ, n_shots=1)

shots = backend.get_result(handle).get_shots()

#store the output bit string in a 1D numpy array
z = shots[0]
